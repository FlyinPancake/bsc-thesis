...
#footnote("BSD had a similar feature: jails for a while; however they were not used by the masses, since Linux did not support it.")
...
==== Docker Compose
Docker comes bundled with a tool called Docker Compose#footnote[This tool is not exclusive for Docker, there is a runtime-agnostic implementation called `oci-runtime-compose`, that is in fact forked from Docker Compose], which allows us to easily define and run multi-container applications.
Without Docker Compose we would have to use the Docker CLI to run each container individually, which can be cumbersome. 
It may lead to hacky shell scripts, resulting in errors, and decreased portability.

With Compose, we use a YAML file to configure our application's services. Then, with a single command, we create and start all the services from our configuration. Compose works in all environments: production, staging, development, testing, as well as CI (Continuous Integration) workflows.#footnote[Usually, the CI workflow has bespoke scripting that uses Compose to build and publish the application's images]
...
Docker Compose is great for single host deployments like development environments, but it falls short when it comes to multi-host deployments.
For example, if we want to deploy our application to many machines, we would have to use a different tool, such as Kubernetes.
...
Sometimes it is beneficial to run a Kubernetes cluster on a single machine, or use the controller node as a worker, but this is not usually recommended for production environments, only in special cases; for example, when redundancy is achieved by other means and not necessary in the cluster.

Preferably, Kubernetes clusters are deployed with multiple controller nodes, which is called a high-availability cluster.
This allows for redundancy, and makes the cluster more resilient to failures.
Worker nodes are usually deployed with multiple nodes too, to allow for horizontal scaling.
...
There are many Kubernetes platforms, which are Kubernetes implementations that are tailored to specific use cases.

Kubernetes ditributions can become certified by the Cloud Native Computing Foundation (CNCF).
This means that the distribution is compliant with the CNCF's standards, and is guaranteed to work with other CNCF certified tools.

Kubernetes platforms can be categorized by their approach to hosting Kubernetes.

==== Hosted Kubernetes
Most cloud providers offer their own Kubernetes hosting solutions, which usually include deep integration with the cloud provider's other services.
They are usually a managed solution, which means that the user does not have to manage the cluster, only the applications running on it.
...
There are also many Kubernetes distributions, which are maintained by the company that created the distribution.
Some of them are open-source and allow the user to audit the code, and even contribute to the project.
Some notable examples are:
...
- Canonical Kubernetes (Charmed Kubernetes) -- Kubernetes distribution by Canonical
- MicroK8s -- Lightweight Kubernetes distribution by Canonical

==== Kubernetes Installers

Kubernetes installers are tools that allow the user to install Kubernetes on a set of machines.
They usually provide a custom Kubernetes distribution, and a set of tools tailored to it, to manage the cluster.
Some notable examples are:
...
The worker nodes run the Pods that are the components of the application workload.
To achieve this, the worker nodes run the following Kubernetes processes:
- `kubelet` -- An agent that runs on each node in the cluster. It makes sure that containers are running in a Pod.
- `kube-proxy` -- `kube-proxy` is a network proxy that runs on each node in your cluster, implementing part of the Kubernetes Service concept.

=== Kubernetes Resources

Kubernetes resources are the building blocks of Kubernetes applications.
...
They are usually used to expose an application to the outside world.
They can be used to expose an application inside the cluster too.
Services define a logical set of endpoints -- usually Pods -- and a policy by which to access them.

There are multiple types of Services:
- `ClusterIP` -- Exposes the Service on a cluster-internal IP. Choosing this value makes the Service only reachable from within the cluster. This is the default `ServiceType`.
- `NodePort` -- Exposes the Service on each Node's IP at a static port (the `NodePort`). A `ClusterIP` Service, to which the `NodePort` Service routes, is automatically created. You'll be able to contact the `NodePort` Service, from outside the cluster, by requesting `<NodeIP>:<NodePort>`. This is similar to `--publish` in `docker run`. `NodeProt` Services can use ports in the range `30000-32767`.
- LoadBalancer -- Exposes the Service externally using a cloud provider's load balancer. `NodePort` and `ClusterIP` Services, to which the external load balancer routes, are automatically created.
- ExternalName -- Maps the Service to the contents of the `externalName` field (e.g. `foo.bar.example.com`), by returning a `CNAME` record with its value. No proxying of any kind is set up. This is useful for transitioning from a `externalName` Service to a `ClusterIP` Service.
...
This will allow the host cluster to schedule the pods.

==== vCluster Control Plane

The vCluster control plane is a set of Kubernetes resources, that are used to manage the virtual cluster.
These resources are: 
- *Kubernetes API server* -- this handles the API requests from `kubectl`
- *Data store* -- the API stores its data in a data store, on real clusters this is usually etcd
- *Controller Manager* -- the controller manager works similarly to the Kubernetes controller manager, however it only manages the virtual cluster's resources
...
To overcome these limitations, vCluster can be configured to use a dedicated scheduler for each virtual cluster.
It can be done by setting these variables in the virtual cluster's `values.yaml` file:
#figure(
caption: "vCluster Scheduler Configuration",
)[
```yaml
sync:
  nodes:
...
If the `PersistentVolumeClaim`-s are synced too, the virtual cluster's scheduler will be able to make storage-aware scheduling decisions.
==== Note on Multi-Namespace Sync
This feature is in alpha state, and is not enabled by default, therefore it was not used for testing in this thesis.

The multi namespace sync feature allows vCluster to create and manage namespaces in the host cluster specific to one virtual cluster.

==== Node Syncing
By default, vCluster will create fake nodes in the virtual cluster, that correspond to the nodes in the host cluster.
If there are multiple nodes in the host cluster, vCluster will only create fake nodes for the nodes, that have pods from the virtual cluster scheduled on them.

There are other options for node syncing:
- *Real Nodes* -- vCluster will copy the nodes' information from the host cluster to the virtual cluster. This will work similarly to the fake nodes, because the virtual cluster will only have nodes for the nodes that have pods from the virtual cluster scheduled on them.
- *Real Nodes All* -- vCluster will copy all nodes from the host cluster to the virtual cluster. This is useful when DaemonSets are used in the virtual cluster.
- *Real Nodes Label Selector* -- vCluster will only sync nodes that match a `key=value` label set on the host cluster nodes. This can be used to create a dedicated node pool for the virtual cluster. To enforce this behavior the `--enforce-node-selector` flag has to be set.
- *Real Nodes + Label Selector* -- vCluster will sync nodes that match the label selector and the information in `spec.nodeName` from vCluster's `values.yaml` file.

== Kubernetes Operator Version Conflict

Especially in case of purpose-built software, it is common to depend on a very specific version of a Kubernetes operator.
This provides a stable environment, where the operator will be bug-for-bug compatible.
However, this can lead to version conflicts when we introduce some other software that depends on a different version of the same operator.
This is a common problem, and in Kubernetes namespaces can be used as a workaround, to isolate the different versions of the operator.
...
Kubernetes is rarely installed on bare metal, and is usually installed on top of some cloud infrastructure.
OpenStack is a popular open-source cloud infrastructure software, that is used by many organizations.
When using OpenStack the maintainer has the option to split the installation into multiple projects, that can be managed separately.
